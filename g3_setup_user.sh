...
if [ ! -f ~/.xscreensaver ]; then ln -s $(pwd)/conf/xscreensaver/xscreensaver ~/.xscreenaver ; fi
...
then
    ...
    echo >> ~/.bashrc
    echo \# Personal Bash configuration extras, added by g3_setup.sh >> ~/.bashrc
...
then
    ...
    if [ ! -d ~/bin ]; then mkdir ~/bin ; fi
    echo >> ~/.bashrc
    echo \# Personal Bash configuration extras, added by g3_setup.sh >> ~/.bash_profile
